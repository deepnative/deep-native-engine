...
export function assignmentAttemptsPage(items: AssignmentAttempt[]) {
  return page(
    ...
    `<section class="error-page"><p class="eyebrow">PRIVATE LOCAL PREVIEW · SAMPLE INFORMATION ONLY</p><h1>Your private assignment attempts</h1><p>Starting, saving and submitting are separate states. A local submission is not reviewed work or evidence of competence.</p>${items.length ? `<ul>${items.map((item) => `<li><a href="/assignments/attempts/${encodeURIComponent(item.id)}">${escape(item.title)}</a> · version ${item.contentVersion} · ${item.submittedAt ? "submitted locally · awaiting future review path" : item.savedAt ? "private draft saved" : "started only"}${item.currentEligible ? "" : " · no longer available for editing"}</li>`).join("")}</ul>` : "<p>No private assignment attempts yet. Choose a published sample from your learning path.</p>"}<p><a href="/learn">Return to your learning path</a></p></section>`,
  );
}
export function assignmentAttemptPage(
  ...
  const responseView = editable
    ? `<form method="post" action="${url}/save">${hidden(csrf)}<input type="hidden" name="revision" value="${item.revision}"><label for="attempt-response">Private sample response</label><textarea id="attempt-response" name="response" maxlength="4000" rows="8">${escape(unsaved ?? item.response)}</textarea><label class="check"><input type="checkbox" name="sample_confirmed" value="yes" required><span>I used only invented or sample information.</span></label><button type="submit">Save private draft</button></form>`
    : unsaved !== undefined
