...
export function trackReadinessPage(snapshot: TrackSnapshot) {
  return page(
    ...
    `<section class="error-page"><p class="eyebrow">LOCAL PREVIEW · NO SERVICE BOOKINGS</p><h1>Learning track readiness</h1><p class="lead">Foundation lessons await qualified curriculum sign-off. Specialist services also need reviewed content and current, verified expert coverage. These states do not grant coaching, formal review or admission.</p><h2>Shared foundation</h2><ul>${snapshot.foundation.map(({ goal, state }) => `<li><strong>${escape(GOALS[goal])}</strong> · ${escape(state)}</li>`).join("")}</ul><h2>IT specialty services</h2><p>No IT specialty service has qualified content and capacity evidence in this preview.</p><ul>${snapshot.itSpecialties.map(({ role, state }) => `<li><strong>${escape(IT_ROLES[role])}</strong> · ${escape(state)}</li>`).join("")}</ul><h2>Specialist domains and services</h2><ul>${snapshot.specialties.map(({ domain, serviceType, state }) => `<li><strong>${escape(DOMAINS[domain])} · ${escape(serviceType)}</strong> · ${escape(state)}</li>`).join("")}</ul><p>General learners may use the separate local preview exercise without a specialist fit review. Tailored service cannot be committed from this page.</p><a href="/readiness">Integration readiness</a></section>`,
  );
}
