...
The executable register is [initial-learning-v3](../../tests/e2e/scenarios.json). Each scenario defines actor, preconditions, steps, expected result, issue links and criticality. Seventeen scenarios run against the real server and PostgreSQL on desktop Chromium and mobile Chromium emulation; each must pass on both to count. The added journeys verify deterministic adapter readiness, private workspace ownership, assignment/support grants and cohort isolation. No external AI, payment, identity or other provider is used.
