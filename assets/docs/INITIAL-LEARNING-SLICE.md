...
The executable register is [initial-learning-v8](../../tests/e2e/scenarios.json). Each scenario defines actor, preconditions, steps, expected result, issue links and criticality. Twenty-seven scenarios run against the real server, PostgreSQL and disposable private object storage on desktop Chromium and mobile Chromium emulation; each must pass on both to count. The newest journey checks honest track preparation and private expert-roster evidence alongside the content workflow. No external AI, payment, identity, storage, scanner or other provider is used.
