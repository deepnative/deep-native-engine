...
The executable register is [initial-learning-v4](../../tests/e2e/scenarios.json). Each scenario defines actor, preconditions, steps, expected result, issue links and criticality. Twenty scenarios run against the real server, PostgreSQL and disposable private object storage on desktop Chromium and mobile Chromium emulation; each must pass on both to count. The added journeys verify deterministic adapter readiness, private workspace ownership, assignment/support grants, cohort isolation, evidence consent/quarantine, authorization-rechecked downloads and coordinated deletion. No external AI, payment, identity, storage, scanner or other provider is used.
