...
The executable register is [initial-learning-v10](../../tests/e2e/scenarios.json). Each scenario defines actor, preconditions, steps, expected result, issue links and criticality. Thirty-one scenarios run against the real server, PostgreSQL and disposable private object storage on desktop Chromium and mobile Chromium emulation; each must pass on both to count. The newest journeys check private, consented sample proposals and private starter plans that fit selected weekly time across general, professional and IT learners while retaining prior practice after a goal change. No external AI, payment, identity, storage, scanner or other provider is used.
