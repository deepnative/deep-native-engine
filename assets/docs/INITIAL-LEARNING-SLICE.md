...
The executable register is [initial-learning-v19](../../tests/e2e/scenarios.json). Each scenario defines actor, preconditions, steps, expected result, issue links and criticality. Fifty scenarios run against the real server, PostgreSQL and disposable private object storage on desktop Chromium and mobile Chromium emulation; each must pass on both to count. The newest journeys check private, consented sample proposals, private time-fitting starter plans, choices from published synthetic assignments, member-owned goal milestones with local-only reminders, accessible exercise-error recovery, optional career plans with unsent member-approved drafts, and three downloadable synthetic workflow demonstrations and private, versioned activity for synthetic lessons, plus local synthetic circle memberships, operator-only aggregate preview metrics, and private versioned synthetic assignment attempts. No external AI, payment, identity, storage, scanner or other provider is used.
