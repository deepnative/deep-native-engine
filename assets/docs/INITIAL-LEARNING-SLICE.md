...
The executable register is [initial-learning-v9](../../tests/e2e/scenarios.json). Each scenario defines actor, preconditions, steps, expected result, issue links and criticality. Twenty-eight scenarios run against the real server, PostgreSQL and disposable private object storage on desktop Chromium and mobile Chromium emulation; each must pass on both to count. The newest journey checks private, consented sample proposals, moderator quarantine and withdrawal redaction alongside the track and content workflows. No external AI, payment, identity, storage, scanner or other provider is used.
