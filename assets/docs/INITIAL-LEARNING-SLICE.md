...
The executable register is [initial-learning-v2](../../tests/e2e/scenarios.json). Each scenario defines actor, preconditions, steps, expected result, issue links and criticality. Fourteen scenarios run against the real server and PostgreSQL on desktop Chromium and mobile Chromium emulation; each must pass on both to count. The fourteenth verifies deterministic adapter readiness. No external AI, payment or other provider is used.
...
This is a loopback-only preview for invented data. It is not production authentication or a deployed member service. Both listener and database address are restricted to loopback; database URL query/fragment overrides are rejected. A random HttpOnly, SameSite=Strict cookie carries browser access, with only its SHA-256 hash stored. Anyone with that browser cookie can access that preview. There is no account recovery or cross-device login. Exact Host/Origin checks, per-session HMAC CSRF, escaped output, CSP and no-store responses address local browser threats. Referrer policy preserves same-origin form Origin headers while withholding cross-origin referrers.

Access expires after 30 days; expiry does not purge stored rows. Clearing cookies loses access, not stored data. Delete this preview removes the current learner and exercises together. Operators may explicitly reset the disposable local database; backups, automatic retention purge and production identity/recovery remain future work. Never enter real client, employer, health or other sensitive content. Cookie Secure/TLS, staff authorization, abuse controls and deployment hardening are required before a hosted service.
...
The register preserves 27 full-MVP requirements: nine original roadmap journeys, ten build-prompt journeys and eight ecosystem additions. Some overlap; they remain explicit until a reviewed requirements decomposition. None is declared complete by this local slice. The complete member-to-qualified-reviewer flow, participation/contributions, production identity and revocation, all browsers/accessibility acceptance, AI budgets/providers, paid entitlements, payments and commercial invariants remain outstanding.

Issue links: [#8](https://github.com/deepnative/deep-native-engine/issues/8), [#15](https://github.com/deepnative/deep-native-engine/issues/15), [#16](https://github.com/deepnative/deep-native-engine/issues/16), [#17](https://github.com/deepnative/deep-native-engine/issues/17), [#18](https://github.com/deepnative/deep-native-engine/issues/18), [#20](https://github.com/deepnative/deep-native-engine/issues/20), [#22](https://github.com/deepnative/deep-native-engine/issues/22), [#24](https://github.com/deepnative/deep-native-engine/issues/24), [#26](https://github.com/deepnative/deep-native-engine/issues/26), [#28](https://github.com/deepnative/deep-native-engine/issues/28), [#33](https://github.com/deepnative/deep-native-engine/issues/33). These are bounded contributions, not completion of their full definitions of done.
