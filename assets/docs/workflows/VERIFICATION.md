...
The application verification runner uses fixed stage diagnostics for setup, evidence parsing and database failures; it never serializes the thrown value. Database deletion, connection shutdown and private-storage cleanup are attempted independently. Any cleanup failure, idle database error or unwritable final report fails the run. Child-process console output and repository Python errors are separate boundaries and are not filtered by this protection. See [the bounded M18 evidence](../CTP-023-SAFE-VERIFICATION-FAILURES.md).
