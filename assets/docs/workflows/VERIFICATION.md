...
The application verification runner uses fixed stage diagnostics for setup, evidence parsing and database failures; it never serializes the thrown value. Database deletion, connection shutdown and private-storage cleanup are attempted independently. Any cleanup failure, idle database error or unwritable final report fails the run. The Python repository runner likewise withholds unexpected exception details, captures Git error output, and replaces input-derived paths/metadata with stable opaque references; intended code-owned gate reasons remain actionable. A failed report write returns nonzero. See [the Python boundary evidence](../CTP-023-SAFE-REPOSITORY-DIAGNOSTICS.md). Unittest traces and inherited child-process output remain separate, unfiltered boundaries. See [the bounded M18 evidence](../CTP-023-SAFE-VERIFICATION-FAILURES.md).
