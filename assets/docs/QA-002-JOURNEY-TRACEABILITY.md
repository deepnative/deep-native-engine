...
Unit probes deliberately corrupt source coverage and browser reports, including missing mapping, skipped/failed/retried tests, wrong browser, duplicate IDs, missing definition, false completion and an incomplete full-release matrix. The release command can now verify the reviewed mapping, but it cannot accept the local-slice report as input or pass with only the seven implemented ECO-01, ECO-03 and ECO-04-A cases. Later scope changes must update the version, preserve provenance and record why any case is split or superseded. No passing local gate, documentation review or overlap table completes the member-to-reviewer, payment, participation or other full-MVP cases.

## Provisional ECO-01, ECO-03 and ECO-04-A execution

The separate `tests/full-e2e/eco-01.spec.ts`, `eco-03.spec.ts` and `eco-04.spec.ts` suites exercise the proposed `F-ECO-01-A/B/C`, `F-ECO-03-A/B/C` and `F-ECO-04-A` IDs in desktop and mobile Chromium against the compiled UI, server and a disposable PostgreSQL database separate from the approved local-slice database. ECO-01 checks exploratory entry, a noncoding exercise and private progress. ECO-03 checks retained private versioned work after a goal change, new-goal eligibility, empty eligible content and recovery from an uncertain save. ECO-04-A checks that joining only one local synthetic circle does not disclose a versioned private practice response to another member of that same circle, including through a forged member-ID query. `make verify` runs these suites after the approved local slice, preserves Playwright traces for all 14 executions, and records their mapped, first-attempt results under `provisionalFullMvpEvidence` in `artifacts/application-verification.json`. Missing, retried, failed, duplicate or unmapped provisional executions fail the shared gate. The provisional evaluator requires every current `requiredChecks` completion token exactly once in each browser's single first-attempt execution, using the same validation as the full-release evaluator; a missing, duplicated or stale token on either browser fails the gate. The tokens follow substantive browser assertions, but a token alone does not prove the assertions or trace are meaningful; ECO-01's six CI traces received separate review in #163, while ECO-03 and ECO-04-A still need their own evidence review. The report records `approved: false`; the local-slice report still shows **0/100** approved full-MVP IDs, and `npm run verify:full-e2e` still requires all 100 cases. The mapping decision does not promote provisional tests to full-release evidence.
