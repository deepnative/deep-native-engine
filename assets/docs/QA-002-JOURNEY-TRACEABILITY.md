...
The command fails closed when the full report is missing, empty, unmapped, incomplete, skipped, failed, retried, duplicated or below coverage. The release target is at least 99% of approved scenario IDs, 100% of critical IDs and 100% pass of executed required tests; the current matrix also requires every registered browser execution, so all 100 IDs must be present. A manually changed `status` cannot substitute for browser evidence. The present `tests/full-e2e/` directory contains twenty-five provisional ROADMAP-01-B, ROADMAP-02-A/B/C/D, ROADMAP-08-A/B, BUILD-02-C/D, BUILD-08-A/B/C, BUILD-10-A/B, ECO-01, ECO-02, ECO-03, ECO-04-A and ECO-06-A cases. Despite mapping approval, a release run remains incomplete until every approved case is implemented and exercised. The local/provisional gates retain their separate denominators.
...
Unit probes deliberately corrupt source coverage and browser reports, including missing mapping, skipped/failed/retried tests, wrong browser, duplicate IDs, missing definition, false completion and an incomplete full-release matrix. The release command can now verify the reviewed mapping, but it cannot accept the local-slice report as input or pass with only the twenty-five implemented ROADMAP-01-B, ROADMAP-02-A/B/C/D, ROADMAP-08-A/B, BUILD-02-C/D, BUILD-08-A/B/C, BUILD-10-A/B, ECO-01, ECO-02, ECO-03, ECO-04-A and ECO-06-A cases. Later scope changes must update the version, preserve provenance and record why any case is split or superseded. No passing local gate, documentation review or overlap table completes the member-to-reviewer, payment, participation or other full-MVP cases.

## Provisional ROADMAP-01-B, ROADMAP-02-A/B/C/D, ROADMAP-08-B, BUILD-02-C/D, BUILD-08-A/B/C, BUILD-10-A/B and ECO journeys

The separate `tests/full-e2e/build-02.spec.ts`, `tests/full-e2e/roadmap-01.spec.ts`, `tests/full-e2e/roadmap-02.spec.ts`, `tests/full-e2e/roadmap-08.spec.ts`, `tests/full-e2e/build-08.spec.ts`, `tests/full-e2e/build-10.spec.ts`, `tests/full-e2e/build-10-a.spec.ts`, `tests/full-e2e/eco-01.spec.ts`, `eco-02.spec.ts`, `eco-03.spec.ts`, `eco-04.spec.ts` and `eco-06.spec.ts` suites exercise the proposed `F-ROADMAP-01-B`, `F-ROADMAP-02-A/B/C/D`, `F-ROADMAP-08-A/B`, `F-BUILD-02-C/D`, `F-BUILD-08-A/B/C`, `F-BUILD-10-A/B`, `F-ECO-01-A/B/C`, `F-ECO-02-A/B/C`, `F-ECO-03-A/B/C` , `F-ECO-04-A` and `F-ECO-06-A` IDs in desktop and mobile Chromium against the compiled UI, server and a disposable PostgreSQL database separate from the approved local-slice database. ROADMAP-01-B checks goal-based common entry, an observed local prerequisite and versioned assignment/rubric choice, then proves that publishing a further lesson creates neither assessed competence nor a completed human review. ROADMAP-02-A checks cross-member workspace, evidence identifier and stolen-link denial plus staff API denial using two synthetic members. ROADMAP-02-B checks that a current reviewer assignment alone grants no evidence read; an exact grant then permits only its clean submitted object, denying another submitted object, an unsubmitted object, an unrelated reviewer and raw workspace drafts. It does not establish qualified human assessment or revision lineage. ROADMAP-02-C revokes the exact reviewer grant and denies both a new link and the issued link, then restores authorization to prove the signed link works just before its five-minute expiry and fails afterward. A test-mode-only capability clock advances the server without forging a signature or waiting five minutes; this is provisional synthetic evidence, not a hosted service decision. BUILD-02-C checks a different synthetic member and an unassigned coach cannot issue or use a submitted member evidence link, while owner access remains. BUILD-02-D revokes the reviewer workspace assignment and denies new and issued links; after a fresh exact assignment, the old signed link works just before its five-minute expiry and fails afterward. The test-mode-only capability clock makes that expiry deterministic. These checks do not publish human feedback or prove evidence revision lineage. ROADMAP-08-B checks that synthetic reviewer de-verification and coach capacity exhaustion appear as separate non-identifying public gaps while specialist booking remains unavailable. ROADMAP-02-D separately checks that a provisioned editor with publication authority cannot read a member’s private draft or clean evidence, obtain a new evidence link or replay the member’s link, while the member retains access. BUILD-08-A checks two synthetic members downloading their own structured local records through the browser; a forged owner-ID query still returns only the requestor's records and an unauthenticated request is denied. This does not establish a complete hosted portability export or authorized operator export. BUILD-08-B now checks an authenticated member revoking the private-review scope through the member-facing evidence form, including the confirmation and changed UI state. The reviewer’s existing and new links are immediately denied while owner access and the separate consent flag remain intact. This is provisional local-browser evidence; other sharing scopes remain separate work. BUILD-08-C now checks the owner’s confirmed deletion through the member-facing evidence form and the removed UI row, then proves one configured active source file, a registered synthetic text-extract derivative, both metadata rows and access through an old owner download link are removed; it does not establish hosted backup or provider-copy erasure. BUILD-10-A checks the synthetic core learning journey at a 375 px viewport and with keyboard-only navigation, visible focus, labelled controls and persisted private self-assessed progress on both browser projects. This is a local Chromium proof, not independent assistive-technology acceptance. BUILD-10-B checks invalid onboarding and exercise values through the browser, field-level recovery and zero invalid exercise rows, then one corrected owner completion; a separate member sees an empty private activity view with a useful starter action and no leaked completion. This is synthetic local-preview behavior, not the full release journey. ECO-01 checks exploratory entry, a noncoding exercise and private progress. ECO-02 checks synthetic editor/reviewer-published reading and task-specific rubric versions, private local submissions for technical and non-IT paths, shared preview access and separate truthful specialist readiness. The IT service list remains in preparation because no role-specific qualified content and capacity registry exists. ROADMAP-08-A independently checks that cybersecurity, product and project-management learners can each enter and complete the common local foundation without specialist approval or a paid Professional grant. ECO-03 checks retained private versioned work after a goal change, new-goal eligibility, empty eligible content and recovery from an uncertain save. Its returning-member check closes the first browser context and re-enters through a fresh context carrying only the same unexpired local preview cookie; it represents a new browser session with no elapsed clock time, not a multi-day persistence test. ECO-04-A checks that joining only one local synthetic circle does not disclose a versioned private practice response to another member of that same circle, including through a forged member-ID query. ECO-06-A checks two distinct goal-specific synthetic lesson sources and unsaved local reflection for general and non-IT professional learners without a career path or live provider. `make verify` runs these suites after the approved local slice, preserves Playwright traces for all 50 executions, and records their mapped, first-attempt results under `provisionalFullMvpEvidence` in `artifacts/application-verification.json`. Missing, retried, failed, duplicate or unmapped provisional executions fail the shared gate. The provisional evaluator requires every current `requiredChecks` completion token exactly once in each browser's single first-attempt execution, using the same validation as the full-release evaluator; a missing, duplicated or stale token on either browser fails the gate. The tokens follow substantive browser assertions, but a token alone does not prove the assertions or trace are meaningful; ECO-01's six CI traces received separate review in #163. The independent #245 review on #94 accepted the synthetic provisional behavior of ROADMAP-01-B, ROADMAP-08-A, ECO-03-A/C, ECO-04-A and ECO-06-A. #246 then corrected ECO-03-B's missing fresh-session boundary; its new first-attempt desktop/mobile traces passed substantive review on #94. Neither review promotes provisional evidence to accepted full-MVP coverage. ROADMAP-02-A/B/C/D, ROADMAP-08-B, BUILD-02-C/D, BUILD-08-A/B/C, BUILD-10-A/B and ECO-02 still need their own evidence review. The report records `approved: false`; the local-slice report still shows **0/100** approved full-MVP IDs, and `npm run verify:full-e2e` still requires all 100 cases. The mapping decision does not promote provisional tests to full-release evidence.
