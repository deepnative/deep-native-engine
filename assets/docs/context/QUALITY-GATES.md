...
- A scenario ID containing several mandatory variants counts only when every variant's action and observable assertion passes in the single first-attempt execution on every registered browser. Two report entries with the same ID/browser cannot combine into coverage; a true case split requires versioned denominator review. See the [QA-002 full-MVP check audit](../QA-002-JOURNEY-TRACEABILITY.md).
