...
- [ ] Run all original applicable source journeys plus ECO-01 through ECO-08 against the seeded private build, covering IT, non-IT professional and general-learner paths, bounded participation/contributions, AI assistance and optional test/manual billing.
...
- [ ] Preserve original privacy, money/date, usability/recovery, late-return and commercial-truth cases while expanding the QA-002 register for the current audience. Report >=99% journey coverage, 100% critical coverage, all required tests passing and >=99% each unit metric on the exact revision/build.
...
Original source: finalized 03-roadmap-and-backlog.md, CTP-024. Original criteria remain in the immutable source archive. The criteria above apply the current audience decision while retaining applicable technical/commercial invariants; supersession is documented in PRODUCT-DIRECTION.md.
