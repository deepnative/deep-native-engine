...
Every released content item has a stable ID and positive version plus owner, rights/source status, review date, supported goals/backgrounds, prerequisites and lifecycle state. Material content changes create a new immutable version. PostgreSQL records the referenced content ID and version with learner work. An old completion must continue to resolve its original content and rubric after a new version is released.
...
- production identity, account recovery and staff authorization design;
