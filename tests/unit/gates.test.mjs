...
it("requires ROADMAP-01-B/02-A/B/C/D/08-A/B, ECO-01 to ECO-04, ECO-06-A/08-C and BUILD-01-A/B/C/D/02-A/C/D/08-A/B/C/10-A/B/D provisional browser evidence without approving release", () => {
  const ids = [
    "F-ROADMAP-02-A",
    "F-ROADMAP-02-B",
    "F-ROADMAP-02-D",
  ...
  expect(assertProvisionalReleaseJourneys(proposal, report)).toMatchObject({
    ...
    criticalPassed: 26,
    criticalTotal: 26,
    executions: 32 * proposal.projects.length,
  });
  for (const [browserIndex, browserName] of proposal.projects.entries()) {
    ...
    for (const [index, id] of [
      ...
      expect(() =>
        assertProvisionalReleaseJourneys(proposal, incomplete),
      ).toThrow(
        new RegExp(`Incomplete required subcases.*${id}.*${browserName}`),
      );
    }
    ...
    const recoveryCaseMissing = copy(report);
    recoveryCaseMissing.suites[0].specs[20].tests[
      browserIndex
    ].results[0].annotations.pop();
    expect(() =>
      assertProvisionalReleaseJourneys(proposal, recoveryCaseMissing),
    ).toThrow(
      new RegExp(`Incomplete required subcases.*F-BUILD-10-B.*${browserName}`),
    );
    const keyboardCaseMissing = copy(report);
    keyboardCaseMissing.suites[0].specs[21].tests[
      browserIndex
    ].results[0].annotations.pop();
    expect(() =>
      assertProvisionalReleaseJourneys(proposal, keyboardCaseMissing),
    ).toThrow(
      new RegExp(`Incomplete required subcases.*F-BUILD-10-A.*${browserName}`),
    );
  }
